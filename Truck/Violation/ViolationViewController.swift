...
class ViolationViewController: BaseViewController {
    let segment = UISegmentedControl.init(items: ["未审批", "已审批", "已驳回"])
    let unconfirmed = ViolationListViewController(flag: 0)
    let confirmed = ViolationListViewController(flag: 1)
    let rejected = ViolationListViewController(flag: 2)
    let button = UIButton()
    ...
    func setupUI() {
        ...
        button.setTitle("上报违章", for: .normal)
